...
// console.log(wrap('aaaaaaaaaaaaaaaa',3))
let wrap=function(width){
	let str = this
	const len = visualLength(str)
	if(len>width){
		return splitByWidth(str,width).join('\n')
	}
	return str
}
function splitByWidth(str,width){
	const len = visualLength(str)
	const times = Math.floor(len/width)
	let res = []
...
export function visualLength(str){
	...
	let len = chars.reduce((res,value)=>value.codePointAt()<255?res+1:res+2,0)
	return len
}
